...
import { getNodeTypeColor, getHttpMethodColor } from '@/lib/utils/colorUtils';

function ApiNodeComponent({ data, selected }: NodeProps<WorkflowNodeData>) {
  const color = getNodeTypeColor('api');
  ...
  return (
    <div
      ...
      aria-label={`API route: ${data.label}`}
    >
      <Handle 
        type="target" 
      ...
      <div className="ml-4 flex-1 min-w-0">
        ...
        <p className="text-[10px] text-gray-400 mt-1 uppercase tracking-wider truncate">
          {data.httpMethod ? `${data.httpMethod} Endpoint` : 'API Route'}
        </p>
      </div>
